import base64
import tkinter as tk
from tkinter import ttk
from concurrent.futures import ThreadPoolExecutor

import functions


class OverwatchRankTracker:

    # global variables
    version = "v0.0.6"
    owner_name = 'enexety'
    repo_name = 'Overwatch_Rank_Tracker'
    token = ""
    path_to_file = 'settings_and_battle_tags.json'

    def __init__(self, widget=None, widget_frame=None):
        self.widget = widget
        self.widget_frame = widget_frame

    @staticmethod
    def run():

        # create main window
        window = tk.Tk()
        window_width = 800
        window_height = 600
        window.title('Overwatch Rank Tracker')
        window.minsize(window_width, window_height)
        window.configure(bg='#2B2B2B')
        window.protocol("WM_DELETE_WINDOW", lambda: functions.exit_main_window(ort=ort))  # custom close
        functions.window_centering(window=window, window_height=window_height, window_width=window_width)

        # set style for window
        style = ttk.Style(window)
        style.theme_use("clam")
        style.configure('Treeview', background='#2B2B2B', foreground="white", fieldbackground="#2B2B2B")

        # set frame for buttons
        button_frame = tk.Frame(window, bg='#2B2B2B')
        button_frame.pack(side='bottom', fill='x')

        # set frame for widgets
        widget_frame = tk.Frame(window, bg="#2B2B2B")
        widget_frame.pack(side="left", fill="both", expand=True)
        ort.widget_frame = widget_frame

        # set buttons
        check_button = tk.Button(button_frame, text='Check', font=('Calibri', 17, 'bold'), pady=3, width=16,
                                 command=lambda: ThreadPoolExecutor().submit(lambda: functions.check_button_click(ort=ort, check_button=check_button, save_button=save_button)))
        save_button = tk.Button(button_frame, text='Save', font=('Calibri', 17, 'bold'), pady=3, width=16, command=lambda: functions.save_button_click(widget=ort.widget, path_to_file=ort.path_to_file))
        settings_button = tk.Button(button_frame, text='Settings', font=('Calibri', 17, 'bold'), pady=3, width=16, command=lambda: functions.open_settings_window(ort=ort, window=window))
        exit_button = tk.Button(button_frame, text='Exit', font=('Calibri', 17, 'bold'), pady=3, width=16, command=lambda: functions.exit_main_window(ort=ort))
        check_button.pack(side='left', fill='x', expand=True)
        save_button.pack(side='left', fill='x', expand=True)
        settings_button.pack(side='left', fill='x', expand=True)
        exit_button.pack(side='left', fill='x', expand=True)

        # set logo image
        logo_base64 = """data:image/png;base64,"""
        logo_image = tk.PhotoImage(data=base64.b64decode(logo_base64.split(",")[1]))
        window.iconphoto(True, logo_image)

        functions.create_loading_text(ort=ort)
        functions.create_text_widget(ort=ort)
        functions.check_for_updates(owner_name=ort.owner_name, repo_name=ort.repo_name, current_version=ort.version, token=ort.token)
        functions.set_settings(path_to_file=ort.path_to_file)

        # launch
        window.mainloop()


if __name__ == "__main__":
    ort = OverwatchRankTracker()
    ort.run()
